...
int Accept(intfd,structsockaddr*sa,socklen_t*salenptr) {
    int n; 
again:
    if ((n = accept(fd, sa, salenptr)) < 0){
	if ()
		r
		    == ECONNABORTED) || (errno == EINTR)) { goto again ;}
...
intSocket(intfamily,inttype,intprotocol) {
    int n;
    if ()
	ocket(family, type, protocol)) < 0) { perr_exit("socket error"); }
    return n;
